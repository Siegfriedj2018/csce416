...
int main(int argc, char *argv[]) {
  ...
  inet_ntop(AF_INET, &client_addr.sin_addr, write_buffer, sizeof(write_buffer));
  std::cout << "Connected to client at ('" << write_buffer << "', '" <<
  ...
  strncpy(write_buffer, server_name.c_str(), sizeof(write_buffer));
  write(server_client_sock_fd, write_buffer, kMAXMESGLEN);

  /* place client name in var */
  client_name = read_buffer;

  /* client stuff */
  while (true) {
    /* Clearing the read and write buffers so it is easier to debug */
    memset(read_buffer, 0, kMAXMESGLEN);
    memset(write_buffer, 0, kMAXMESGLEN);
